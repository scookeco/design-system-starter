...
describe('schema-driven pages', () => {
  ...
  it('an account page lists the records that point at it by id, with rollups from the same counts', async () => {
    ...
    expect(await screen.findByRole('heading', { level: 1, name: account?.name ?? '?' })).toBeTruthy();
  ...
  it('a person page lists the records they own', async () => {
    ...
    expect(await screen.findByRole('heading', { level: 1, name: person?.name ?? '?' })).toBeTruthy();
  ...
  it('edits from the same form, starting from the server’s values; the new name shows in every record row', async () => {
    ...
    history.push('/records?display=board');
    await waitFor(() => expect(screen.getAllByRole('link', { name: 'Northwind Holdings' }).length).toBeGreaterThan(0));
