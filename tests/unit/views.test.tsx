...
describe('saved views on the list page', { timeout: PAGE_FLOW_TIMEOUT }, () => {
  it('choosing a view writes its config into the URL; changing anything shows Modified; Save changes stores it', async () => {
    ...
    await screen.findByRole('combobox', { name: 'Saved view' }, FIRST_PAINT);
    // The views have loaded when the placeholder stops saying so; look the trigger up again rather than hold a node across renders.
    const savedView = () => screen.getByRole('combobox', { name: 'Saved view' });
    await waitFor(() => expect(savedView().textContent).toBe('Unsaved view'));
    fireEvent.keyDown(savedView(), { key: 'Enter' });
  ...
  it('opens the default view when the list opens with nothing in its URL, and never over a shared link', async () => {
    ...
    await waitFor(() => expect(listCodec.parse(bare.history.location().search)).toMatchObject({ saved: 'acme-v2', display: 'board', sort: '-updated' }), FIRST_PAINT);
