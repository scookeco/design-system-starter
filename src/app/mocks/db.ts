 ...
 */
import { TENANTS, type Person, type RecordEntity, type Tenant } from '../api/schemas';
import { SEED_EPOCH, seedPeople, seedRecords } from './seed';

interface Partition {
  records: RecordEntity[];
  people: Person[];
  /** Idempotency-Key → the record that request created. A replayed create returns it again. */
  created: Map<string, RecordEntity>;
  nextId: number;
}

const fresh = (tenant: Tenant): Partition => {
  const records = seedRecords(tenant);
  return { records, people: seedPeople(tenant), created: new Map(), nextId: 1001 + records.length };
};
